...
it_can_put_and_bump_first_version() {
  ...
  put_uri_with_bump $repo $src pre-release alpha | jq -e "
    .version == {number: \"0.0.1-alpha.1\"}
  "
  ...
  test -e $repo/some-file
  test "$(cat $repo/some-file)" = 0.0.1-alpha.1
}
...
it_can_put_and_bump_first_version_with_initial() {
  ...
  put_uri_with_bump_and_initial $repo $src 1.2.3 pre-release alpha | jq -e "
    .version == {number: \"1.2.4-alpha.1\"}
  "
  ...
  test -e $repo/some-file
  test "$(cat $repo/some-file)" = 1.2.4-alpha.1
}
...
it_can_put_and_bump_with_message_over_existing_version() {
  ...
  put_uri_with_bump_and_message $repo $src pre-release alpha "$message" | jq -e "
    .version == {number: \"1.2.4-alpha.1\"}
  "
...
it_can_put_and_bump_with_message_and_replace_over_existing_version() {
  ...
  local expected_message="This is a commit message on some-file with 1.2.4-alpha.1"
